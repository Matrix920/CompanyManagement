...
<head>
 ...
 <%@page import="controller.DbAdmin" %>
    ...
    <%
        ...
        */
        ...
        if(username!=null && password!=null){
            ...
            Client client=DbClient.login(username, password);
            
            if(client.client_id!=0){
                session.setAttribute(Client.IS_LOGIN, true);
                session.setAttribute(Client.ROLE, Client.CLIENT);
                session.setAttribute(Client.CLIENT_ID, client.client_id);
                response.sendRedirect("index.jsp");
            }else{
                Admin admin=DbAdmin.login(username, password);
                if(admin.admin_id!=0){
                    session.setAttribute(Client.IS_LOGIN, true);
                    session.setAttribute(Client.ROLE, Client.ADMIN);
                    response.sendRedirect("index.jsp");
                }
            }
                ...
                <div class="row">
                    ...
                    <div class="site_information">
                        <ul>
                            ...
                            <li><a class="join_bt" href="Login.jsp">تسجيل الدخول</a></li>
                        </ul>
                    </div>
                </div>
            </div>
